...
#TOMLファイルから接続情報を読み込む
try:
    config = toml.load('kimura_connections.toml')
    sf = config['snowflake']
...
for sql_file in sql_all_files:
    try:
        ...
        df['データがフィックスされたタイミング'] = pd.to_datetime(df['データがフィックスされたタイミング'])
        df['データがフィックスされたタイミング'] = df['データがフィックスされたタイミング'].dt.strftime('%Y-%m-%d %H:%M:%S.%f %z')
        df['データがフィックスされたタイミング'] = df['データがフィックスされたタイミング'].str.replace(r'(\.\d{3})\d{3}', r'\1', regex=True)
