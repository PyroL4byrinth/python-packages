...
if os.path.exists(OUT_EVENTS) and os.path.getsize(OUT_EVENTS) > 0:
    ex = pd.read_csv(OUT_EVENTS, encoding=ENC)
    if not ex.empty:
        ex['TIME'] = pd.to_datetime(ex['TIME'], errors='coerce')
        ex = ex.dropna(subset=['TIME'])

        # 
        latest = ex.sort_values('TIME').groupby('Name', as_index=False).tail(1)
        for _, r in latest.iterrows():
            open_x[r['Name']] = [r['TIME']]
...
if rem:
    ...
    # add 20260312 
    rem_latest_df = rem_df.loc[rem_df.groupby("Name")["TIME"].idxmax()]
    rem_latest_df.to_csv(OUT_EVENTS, index=False, encoding=ENC)
else:
